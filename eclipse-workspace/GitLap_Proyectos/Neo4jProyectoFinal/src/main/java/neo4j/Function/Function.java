...
public class Function {
	...
	public static void rebuildDB() {
		try ( Session session = Conexion.getDriver().session() ) {
	...
	public static int countRelationship(String relationship) {
		try ( Session session = Conexion.getDriver().session() ) {
	...
	public static void nodeRelationshipNode(Transaction tx, final String learn, final String node, final String attribute, final String property, final String description) {
		...
		if (num != 0.0f) tx.run( "MATCH (x:" + learn +" {nombre : $learn}) "
								...
								parameters( "learn", learn, "node", node, "description", description));
	}
	 ...
	 * @param relationship
	 */
	private static void deleteRelationship(final String relationship) {
		try ( Session session = Conexion.getDriver().session() ) {
            session.writeTransaction( new TransactionWork<Boolean>() {
                @Override
                public Boolean execute(Transaction tx) {
	 ...
	 * @param node
	 */
	public static void deleteNode(final String node) {
		deleteRelationship(node);
		try ( Session session = Conexion.getDriver().session() ) {
            session.writeTransaction( new TransactionWork<Boolean>() {
                @Override
                public Boolean execute(Transaction tx) {
