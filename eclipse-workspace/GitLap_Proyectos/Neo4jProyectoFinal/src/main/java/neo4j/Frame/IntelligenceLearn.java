...
public class IntelligenceLearn extends JPanel {
	...
	private final JButton btn_learn = new JButton("Aprender");
	...
	public IntelligenceLearn() {
		...
		btn_insertar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (Conexion.getUsuario().equals("admin") || Conexion.getUsuario().equals("editor")) {
					if (text_descripcion.getText().length() != 0)
						insertarTabla(whoIsVisible(), getIdentificador());
				} else {
					Thread hilo = new Thread(() -> {
						Color color = btn_insertar.getBackground();
						btn_insertar.setBackground(Color.red);
						try {
							Thread.sleep(2000);
						} catch (InterruptedException e1) {}
						btn_insertar.setBackground(color);
					});
					hilo.start();
				}
			}
		...
		btn_learn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				Thread hilo = new Thread(() -> {
					Color color = btn_learn.getBackground();
					btn_learn.setBackground(Color.red);
					try {
						Thread.sleep(2000);
					} catch (InterruptedException e1) {}
					btn_learn.setBackground(color);
				});
				hilo.start();
				}
			}
